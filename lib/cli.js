...
async function doPublishStatus (pw, opts) {
  for (let {repo, pkg, commits} of await pw.publishStatusForPackages(opts)) {
    ...
    // but another process prepends "Release", so remove
    // we cannot coerce the semver or any number will be taken as valid
    const message = (commits[0] || {message: ''}).message.replace(/^Release\s/, '');
