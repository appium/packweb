...
class PackWeb {
  ...
  async infoCmd (pkg) {
    ...
    try {
      ...
      console.error(`Package ${pkg} could not be accessed`);
    }
  }
  ...
  async repoForPackage (p) {
    ...
    if (!gitRe.test(info.repository.url)) {
      console.error(`Don't know what to do with non-github urls right now, for package ${p}`);
      return null;
    }
  ...
  async updateOwnersForPackage (p) {
    ...
    if (stats.validOwners.length > 0 &&
        !_.contains(stats.validOwners, this.npmUser)) {
      ret.denied = true;
      return ret;
    }
